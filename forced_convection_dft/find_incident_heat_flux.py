...
class HeatFluxFinder:
    def __init__(self, temperature_front, temperature_back, temperature_thermocouple, ambient_temperature,
        ...
        if isinstance(ambient_temperature, float) or isinstance(ambient_temperature, int) or len(ambient_temperature) == 1:
            self.ambient_temperature = ambient_temperature * np.ones_like(time_vector)
        else:
    ...
    def find_heat_transfer_coefficients(self, velocity):
        ...
        h_back_dft = heat_transfer_coefficients.natural_convection(
            (self.temperature_back + self.ambient_temperature) / 2, Kelvin=True,
