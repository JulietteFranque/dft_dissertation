...
class ForwardModelThermocouple:
    def __init__(self, time, q_inc, characteristic_velocity, temp_init,
        ...
        if isinstance(temp_amb, float) or isinstance(temp_amb, int) or len(temp_amb) == 1:
            self.temp_amb = temp_amb * np.ones_like(time)
        else:
    ...
    def run(self):
        ...
        for t in tqdm(range(n_time - 1)):
            h = SphereCorrelations(
                ...
                ambient_temperature=self.temp_amb[t],
...
class ForwardModelDft:
    ...
    def _initialize_arrays(self):
        ...
        self.h_f[0], self.h_b[0] = self._find_heat_transfer_coefficients(
            ...
            self.characteristic_velocity[0], self.temp_amb[0])
    ...
    def _calculate_d_temp_d_time_plate(self, temp_plate, adjacent_slab_temp,
                                       ...
                                       epsilon, q_inc, h, temp_amb):
        q_ins_minus_ref = epsilon * q_inc
        q_conv = h * (temp_plate - temp_amb)
    ...
    def _find_heat_transfer_coefficients(self, temp_f, temp_b, velocity, temp_amb):
        ...
        back_heat_transfer_coefficient = natural_convection(
            (temp_b + temp_amb).reshape(-1, 1) / 2, Kelvin=True,
            T_infty=temp_amb, L_ch=self.plate_side_length).custom(0.65, 0.25)
    ...
    def _update_heat_transfer_coefficient(self, time_step_number):
        self.h_f[time_step_number + 1], self.h_b[
            ...
            self.characteristic_velocity[time_step_number], self.temp_amb[time_step_number])
    ...
    def _construct_insulation_matrix(self, fourier_number):
        k = [np.ones(self.n_nodes - 1) * fourier_number,
