...
class physical_models:
    """
    Common methods among physical forced_convection_dft
    """
...
class one_dim_conduction(physical_models):
    ...
    def one_d_conduction(self):
        ...
        def construct_banded_matrix(Fo=5, nodes=10):
            ...
            """
            k = [np.ones(nodes-1)*(-Fo), np.ones(nodes)*(1+2*Fo), np.ones(nodes-1)*(-Fo)]
        ...
        self.q_cond_if = -self.insul.k*(T_nodes_i[:,0] - self.T_f)/delta_x
        self.q_cond_ib = -self.insul.k*(self.T_b - T_nodes_i[:,-1])/delta_x
        self.T_nodes_i = T_nodes_i
